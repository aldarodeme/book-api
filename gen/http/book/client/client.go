...
type Client struct {
	...
	// GetOne Doer is the HTTP client used to make requests to the GetOne endpoint.
	GetOneDoer goahttp.Doer
...
// GetOne returns an endpoint that makes HTTP requests to the book service
// GetOne server.
func (c *Client) GetOne() goa.Endpoint {
	var (
		decodeResponse = DecodeGetOneResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v any) (any, error) {
		req, err := c.BuildGetOneRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.GetOneDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("book", "GetOne", err)
		}
		return decodeResponse(resp)
	}
}
